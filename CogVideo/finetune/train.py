...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="/inspire/hdd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/wangyibin-240108120105/CogVideo/CogVideoX-2b",
        # required=True,
    ...
    parser.add_argument(
        "--instance_data_root",
        type=str,
        default="/inspire/hdd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/wangyibin-240108120105/CogVideo/create_caption/40b_sifted_dataset",
        help=("A folder containing the training data."),
    )
    parser.add_argument(
        "--data_json",
        type=str,
        default="/inspire/hdd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/wangyibin-240108120105/CogVideo/create_caption/13b_sifted_dataset/sifted_reward_data_13b.json",
        help=("A folder containing the data json."),
    )
    parser.add_argument(
        "--video_column",
        type=str,
...
class VideoDataset(Dataset):
    def __init__(
        ...
        # data format should follow: "/path/data1.json+/path/data2.json" 
        data_json_list = data_json.split('+')
        self.instance_prompts = []
        self.instance_video_paths = []
        self.instance_score = []
        for data_json_ in data_json_list:
            with open(data_json_, "r", encoding="utf-8") as file:
                data_list = json.load(file)
                self.instance_prompts.extend([data['caption'] for data in data_list])
                
                self.instance_video_paths.extend([self.instance_data_root.joinpath('videos/'+'/'.join(data['video'].split('/')[-2:])) for data in data_list])
                
                self.instance_score.extend([float(data['score']) for data in data_list])
    ...
    def __getitem__(self, index):
        return {
            ...
            "instance_video_path": self.instance_video_paths[index],
            "instance_score": self.instance_score[index]
        }
...
def main(args):
    ...
    def collate_fn(examples):
        ...
        for example in examples:
            filename=example['instance_video_path']
            video_reader = decord.VideoReader(uri=filename.as_posix(), width=args.width, height=args.height)
            video_num_frames = len(video_reader)

            start_frame = min(args.skip_frames_start, video_num_frames)
            end_frame = max(0, video_num_frames - args.skip_frames_end)
            if end_frame <= start_frame:
                frames = video_reader.get_batch([start_frame])
            elif end_frame - start_frame <= args.max_num_frames:
                frames = video_reader.get_batch(list(range(start_frame, end_frame)))
            else:
                indices = list(range(start_frame, end_frame, (end_frame - start_frame) // args.max_num_frames))
                frames = video_reader.get_batch(indices)
            ...
            frames = torch.stack([train_transforms(frame) for frame in frames], dim=0)
            # videos.append(frames.permute(0, 3, 1, 2).contiguous())  # [F, C, H, W]
            example['instance_video'] = frames.permute(0, 3, 1, 2).contiguous()
            example['instance_video'] = encode_video(example["instance_video"])

        videos = [example["instance_video"].sample() * vae.config.scaling_factor for example in examples]
        prompts = [example["instance_prompt"] for example in examples]
        scores = [example["instance_score"] for example in examples]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                loss = torch.mean((weights * (model_pred - target) ** 2).reshape(batch_size, -1), dim=1) * torch.Tensor(scores).cuda()
                loss = loss.mean()
